    ...
    t@(LC.And p q) -> do
      ...
      if x' == zero
        then do
          ...
          let m2 = Map.singleton c $
                     Map.singleton s2
                       (ri',
                        SubexpressionTrace zero
...
lookupTermSubexpressionTraces ann lc arg mapping tables c ri lookupArg outCol = do
  (ri', inputs') <-
    second (Map.fromList
      . zip ((^. _2) <$> lookupArg))
      <$> foldM (\(ri', xs) -> fmap (\(ri'', a, b, d) -> (ri'', xs <> [(a,b,d)]))
                             . term ri'
                             . (^. #getInputExpression)
                             . (^. _1))
                (ri, [])
